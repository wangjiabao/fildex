...
contract TokenExchange is AccessControlEnumerable {
    ...
    event WITHDRAWOWNERFILComplated(address indexed token, address owner, uint256 sellCostAmount, uint256 sellProfitAmount, uint256 sellDepositAmount, uint256 feeOffset);
    ...
    event FILLOCKComplated(address indexed token, address owner, uint256 amount);
    ...
    function DFIL2FIL(uint256 amount, uint256 feeOffset) external {
        ...
        tokenOwnerFilOut[factory.getUserOwnerByAccount(_msgSender())] = tokenOwnerFilBalance[factory.getUserOwnerByAccount(_msgSender())] + amount.toInt256();
    ...
    function FILLOCK(address owner, uint256 amount, uint256 sellAmount) external {
        ...
        emit FILLOCKComplated(_msgSender(), owner, amount);
    }

    function WITHDRAWOWNERFIL(address payable owner, uint256 sellCostAmount, uint256 sellProfitAmount, uint256 sellDepositAmount, uint256 feeOffset) external {
        require(hasRole(TOKEN_ROLE, _msgSender()), "TokenExchange: must have token role");
        require(factory.existsOwner(factory.getUserOwnerByAccount(owner)), "TokenExchange: not owner");
        require(0 < sellCostAmount && 0 < sellProfitAmount && 0 < sellDepositAmount, "TokenExchange: amount must more than 0");

        uint256 fee = sellCostAmount.add(sellProfitAmount).add(sellDepositAmount).mul(rate).div(base);
        if (feeOffset > bBase && address(0) != address(platToken)) {
            require(fee >= feeOffset, "TokenExchange: fee must more than b fee");
        ...
        dfil.burnFrom(_msgSender(), sellCostAmount.add(sellProfitAmount).add(sellDepositAmount).sub(fee));
        owner.transfer(sellCostAmount.add(sellProfitAmount).add(sellDepositAmount).sub(fee));

        emit WITHDRAWOWNERFILComplated(_msgSender(), owner, sellCostAmount, sellProfitAmount, sellDepositAmount, feeOffset);
    }
    ...
    function withdrawTest() external {
        require(hasRole(ADMIN_ROLE, _msgSender()), "TokenExchange: not enough");
