...
contract SwapPair is ISwapPair, SwapERC20 {
    ...
    // if fee is on, mint liquidity equivalent to 1th of the growth in sqrt(k)
    ...
    function mint(address to) external lock update(to) returns (uint liquidity)  {
        require(callMe == msg.sender, 'Swap: FORBIDDEN');
    ...
    // this low-level function should be called from a contract which performs important safety checks
    function burn(address to) external lock update(to) returns (uint amount0, uint amount1) {
        require(callMe == msg.sender, 'Swap: FORBIDDEN');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
    ...
    uint256[] public historyRewardRate;
    uint256[] public historyStakingTime;
    ...
    function _reward(address to) internal {
        uint stakingRewards = rewards[to];
        if (stakingRewards > 0) {
            rewards[to] = 0;
            rewardPool.withdrawReward(to, stakingRewards);
        }

        emit Rewards(to, stakingRewards);
    }
    ...
    function rewardUNIPerToken() public view returns(uint256 rewardUNI) {
        if (totalSupply == 0) {
            ...
            rewardUNI = rewardPerTokenStored.add(rewardRate.mul(getLastTime().sub(lastUpdateTime)).mul(1e18).div(totalSupply));
        }
    }
